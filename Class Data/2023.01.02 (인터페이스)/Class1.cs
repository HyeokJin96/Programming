...
namespace _2023._01._02__인터페이스_
{
    public class 인터페이스
    {
         ...
         * 전체 프로그램의 설계도에 대한 명세서를 정성할 수 있다. 이 강의에서는 프로그램의
         * 표준 규약을 정하고 따를 수 있도록 강제하는 인터페이스(Interface)를 학습해보자.
         * 인터페이스는 클래스에서 구현해야 하는 관련 기능의 정의가 포함된 개념이다.
         * 
         * 인터페이스
         * 인터페이스는 클래스 또는 구조체에 포함될 수 있는 관련있는 메서드들을 묶어 관리한다.
         * 인터페이스는 명세서(규약, 표준) 역할을 한다.
         * 인터페이스를 상속받아 그 내용을 구현하는 클래스는 인터페이스에 선언된
         * 맴버(속성, 메서드 등)가 반드시 구현되어 있다는 보증을 한다.
         * 
         *  - 인터페이스는 interface 키워드를 사용하여 만든다. 인터페이스는 실행 가능한 코드와
         *    데이터를 포함하고 있다.
         *  - 추상 클래스처럼 다른 클래스에 맴버 이름을 미리 정의할 때 사용한다.
         *    추상 클래스와 다른 점은 맴버 내용을 구현하지 않고 맴버 이름만 정의한다.
         *  - 인터페이스에는 메서드, 속성, 인덱서 및 이벤트를 정의할 수 있다.
         *  - 현실 세계에서 전 세계 표준과 같은 기능이다.
         *  - 단일 상속만 지원하는 클래스와 달리 인터페이스를 사용한 다중 상속이 가능하다.
         *  - 인터페이스 맴버는 엑세스 한정자를 붙이지 않으며 항상 public이고,
         ...
         * C#에서 클래스는 클래스에 대한 단일 상속만 지원하는 대신, 인터페이스는 클래스에 인터페이스를
         * 하나 이상 상속할 수 있다.
         * 
         * 인터페이스와 추상 클래스 비교하기
         ...
         * 
         * 추상 클래스
         *  - 구현된 코드가 들어온다. 즉, 메서드 시그니쳐만 있는 것이 아니라 사용 가능한 실제로 구현된
         *    메서드도 들어온다.
    ...
    class car : Icar
    {
        public virtual void Go()
        {
            ...
            Console.WriteLine("인터페이스를 사용할 때는 정의된 모든 맴버를 반드시 구현해야 한다. ");
        }
    }
